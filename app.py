...
SPYRO_AVATAR = (
    "https://raw.githubusercontent.com/twitter/twemoji/master/assets/72x72/1f409.png"
)
USER_AVATAR = "👤"

st.title("🐉 Spyro Scientific Assistant")
st.caption(
    "Sistema Conversacional RAG | FAISS + Re-ranking (Cross-Encoder) + Gemini"
    " 2.0 Flash"
)
...
# -----------------------------------------------------------------------------
@st.cache_resource(
    show_spinner="Cargando modelos pesados y base vectorial FAISS..."
)
def load_resources():
...
def traducir_consulta_a_ingles(query):
  """Traduce la consulta a inglés únicamente para la búsqueda en FAISS."""
  try:
    response = ai_client.models.generate_content(
        model="gemini-2.0-flash",
        contents=(
            "Translate the following search query to English. Return ONLY the"
            f" translated string without extra quotes or formatting: {query}"
        ),
    )
...
def retrieve_and_rerank(query, fetch_k=20, top_n_final=3):
  ...
  return sorted(candidatos, key=lambda x: x["rerank_score"], reverse=True)[
      :top_n_final
  ]
...
def generate_rag_response(query, contexts):
  """Fase G: Generación fluida en español basada en el contexto."""
  if not contexts:
    return (
        "El corpus no contiene información suficiente para responder a esta"
        " consulta."
    )
  ...
  system_instruction = (
      "Eres Spyro, un asistente científico experto. Lee y analiza el contexto"
      " proporcionado (en inglés) y responde a la pregunta del usuario"
      " estrictamente en ESPAÑOL utilizando ÚNICAMENTE el contexto.\n\nReglas"
      " obligatorias:\n1. La respuesta DEBE estar redactada completamente en"
      " ESPAÑOL de manera clara, fluida, natural y profesional.\n2. Basa tu"
      " respuesta de manera precisa en la información de los documentos.\n3."
      " Si el contexto NO contiene información suficiente para responder la"
      " consulta de manera certera, DEBES responder exactamente: 'El corpus no"
      " contiene información suficiente para responder a esta consulta.'\n4."
      " No inventes ni asumas información ajena al contexto."
  )
...
if "messages" not in st.session_state:
  st.session_state.messages = [{
      "role": "assistant",
      "content": (
          "¡Hola! Soy **Spyro**, tu asistente científico. Hazme cualquier"
          " consulta sobre el corpus y te responderé en español respaldado por"
          " evidencias exactas."
      ),
      "sources": [],
  }]
...
if user_input := st.chat_input("Escribe tu pregunta aquí..."):
  ...
  with st.chat_message("assistant", avatar=SPYRO_AVATAR):
    with st.spinner("Spyro está analizando los artículos científicos..."):
      ...
      # 3. Generar respuesta formal con Gemini
